import { z } from 'zod'
import { CLEANER_REJECTION_REASON_OPTIONS } from '@/lib/house-sitter-status'
...
export const updateCleanerSchema = z.object({
  ...
  cleaning_supplies: z.enum(['own_supplies', 'house_sit_supplies', 'house_sitter_brings', 'house_sit_provides']).optional().nullable(),
...
export const approveCleanerSchema = z.object({
  ...
  if (!hasLegacy && !hasCode && !hasCustom) {
    ctx.addIssue({
      ...
      message: 'Rejection reason is required when rejecting a houseSitter.',
    })
  }
})

export const houseSitterSearchSchema = z.object({
