...
import type { BookingRead, BookingStatus, HouseSitterOnboardingProgress } from '@/types'
...
export default function CleanerDashboardPage() {
  ...
  async function submitForApproval() {
    ...
    try {
      await houseSittersApi.submitForApproval()
  ...
  const missingOnboardingParts = useMemo(() => {
    ...
    const labels: Array<[keyof HouseSitterOnboardingProgress['steps'], string]> = [
        ...
        <div className="relative z-10 grid gap-3 px-5 py-4 sm:px-6 sm:py-5 lg:grid-cols-[1.2fr_0.8fr] lg:items-end lg:px-8 lg:py-6">
          <div className="animate-stage-up space-y-4">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <Link
                href="/house-sitters/bookings?status=pending"
      ...
      {lifecycleStatus === 'rejected' ? (
        <div className="rounded-2xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50 px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link href="/house-sitters/profile" className="inline-flex h-8 items-center rounded-xl bg-primary px-3 text-xs font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:opacity-95 shrink-0">
              Update profile
            </Link>
          </div>
        </div>
        ...
        <Card className="internal-card xl:col-span-2 border-slate-200">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Link href="/house-sitters/bookings?status=pending" className="inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
                View all
                ...
                <div key={b.id} className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                  ...
                  <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="w-full sm:w-auto sm:text-right">
                      ...
                      <Link
                        href={`/house-sitters/bookings/${b.id}`}
        ...
        <div className="space-y-4">
          <Card className="internal-card border-slate-200">
            ...
            <CardContent className="space-y-2">
              <Link href="/house-sitters/profile?tab=availability" className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50">
                <span className="inline-flex items-center gap-2"><CalendarClock className="h-4 w-4 text-primary" />Update availability</span>
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/house-sitters/chats" className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50">
                <span className="inline-flex items-center gap-2"><MessageSquare className="h-4 w-4 text-primary" />Open chats</span>
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/house-sitters/profile?tab=reviews" className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50">
                <span className="inline-flex items-center gap-2"><Star className="h-4 w-4 text-primary" />Manage reviews</span>
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/house-sitters/profile?tab=payments" className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50">
                <span className="inline-flex items-center gap-2"><Euro className="h-4 w-4 text-primary" />Payments &amp; payouts</span>
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/house-sitters/report" className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50">
            ...
            <CardContent className="space-y-3">
              {stats.upcoming.length === 0 ? (
                ...
                stats.upcoming.slice(0, 3).map((b) => {
                  ...
                  return (
                    <Link key={b.id} href={`/house-sitters/bookings/${b.id}`} className="block rounded-xl border border-slate-200 bg-slate-50 p-3 hover:bg-slate-100">
      ...
      <Card className="internal-card border-slate-200">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <Link href="/house-sitters/bookings" className="rounded-md px-2 py-1 text-sm font-medium text-primary hover:bg-primary/10">
              Open bookings
            </Link>
          </div>
        </CardHeader>
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
              {bookings.slice(0, 6).map((b) => {
                ...
                const proposalSummary = isAmendProposal
                  ...
                  : `${proposalActor} proposed: ${formatDate(b.scheduled_start)} → ${formatDate(b.proposed_start ?? b.scheduled_start)}`
                return (
                  <Link key={b.id} href={`/house-sitters/bookings/${b.id}`} className="rounded-xl border border-slate-200 bg-white p-3 hover:border-primary/40 hover:shadow-sm">
