...
import { houseSittersApi, availabilityApi, paymentsApi, usersApi, phoneVerificationApi } from '@/lib/api'
...
import type { HouseSitterOnboardingProgress, HouseSitterRead } from '@/types'
...
function HouseSitterOnboardingPageContent() {
  ...
  async function saveStep1() {
    ...
    setSaving(true)
    try {
      const res = await houseSittersApi.updateMyOnboarding({
  ...
  async function continueToQuiz() {
    ...
    setSaving(true)
    try {
      await houseSittersApi.updateMyOnboarding({
        onboarding_step: 5,
              ...
              {step5Mode === 'success' && (
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 space-y-2">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <Button onClick={() => router.push('/house-sitters/dashboard')}>Go to dashboard</Button>
                  </div>
                </div>
              )}
            </div>
