...
export default function HouseSitBookingDetailPage() {
  ...
  async function handleCancelRequest() {
    ...
    try {
      ...
      await refresh()
      router.push('/house-sits/bookings')
    } catch (err: any) {
      ...
      <div className="houseSit-booking-detail-revamp space-y-7 md:space-y-9">
        ...
        <div>
          <Button
            ...
            onClick={() => router.push('/house-sits/bookings')}
          >
            ...
            <Card className="border-slate-200 bg-white/90">
              <CardContent className="space-y-2 px-5 pb-5 pt-6 sm:px-6 sm:pb-6 sm:pt-6">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
                  ...
                  {canAuthorize && (
                    <Button size="lg" className="w-full sm:w-auto" onClick={() => router.push(`/house-sits/checkout/${id}`)}>
                      Authorise card
                    </Button>
                  )}
                  {canContinuePayment && (
                  ...
                  {canReportInProgress && (
                    <Button variant="destructive" className="w-full sm:w-auto" onClick={() => router.push(`/house-sits/report?booking=${id}`)}>
                      Report a problem
                    </Button>
                  )}
                  {isCompletedReleased && (
