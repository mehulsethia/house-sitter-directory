...
import { bookingsApi, houseSitsApi, paymentsApi, phoneVerificationApi } from '@/lib/api'
...
export default function ClientProfilePage() {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      await houseSitsApi.updateMe(profilePayload)
      if (!phoneChanged) {
  ...
  async function createSavedAddress() {
    ...
    try {
      const createdRes = await houseSitsApi.addAddress({
  ...
  function startEditAddress(entry: HouseSitAddressRead) {
  ...
  async function setAddressAsDefault(entry: HouseSitAddressRead) {
    if (entry.is_default) return
    setUpdatingAddress(true)
    try {
      const res = await houseSitsApi.updateAddress(entry.id, { is_default: true })
  ...
  async function saveEditedAddress() {
    ...
    setUpdatingAddress(true)
    try {
      const res = await houseSitsApi.updateAddress(editingAddressId, {
  ...
  async function uploadClientIdDocument(file: File) {
    ...
    try {
      ...
      const res = await fetch(toApiV1Url('/upload/house-sit-id-document'), {
        method: 'POST',
    ...
    <>
      <div className="client-profile-revamp space-y-7 md:space-y-9">
        ...
        <section className="grid gap-4 lg:grid-cols-[340px_1fr]">
          <div className="rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-5 shadow-[0_16px_38px_rgba(74,56,39,0.12)] backdrop-blur-sm">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await houseSitsApi.deleteAddress(confirmDeleteAddressId)
