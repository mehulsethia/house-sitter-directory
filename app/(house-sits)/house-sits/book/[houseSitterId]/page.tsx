...
import { houseSittersApi, bookingsApi, availabilityApi, houseSitsApi, paymentsApi, phoneVerificationApi } from '@/lib/api'
...
import type { HouseSitterRead, PriceBreakdown, BookingRead, HouseSitProfileRead, HouseSitAddressRead } from '@/types'
  ...
  async function handleSubmit() {
    ...
    try {
      const { error } = await stripe.confirmPayment({
        ...
        confirmParams: {
          return_url: `${window.location.origin}/house-sits/bookings`,
        },
      })
      if (error) {
...
export default function BookingFlowPage() {
  const { houseSitterId } = useParams<{ houseSitterId: string }>()
  ...
  function draftStorageKey() {
    return `booking_flow_draft_v${BOOKING_FLOW_DRAFT_VERSION}:${houseSitterId}`
  }
  ...
  function navigateToStep(nextStep: number, options?: { dropResumeParams?: boolean; push?: boolean }) {
    ...
    const nextUrl = `/house-sits/book/${houseSitterId}${qs.toString() ? `?${qs.toString()}` : ''}`
    if (options?.push) {
      router.push(nextUrl)
    } else {
  ...
  function clearSessionDraft() {
    clearLocalDraft()
    bookingsApi.clearFlowDraft(houseSitterId).catch(() => null)
  }
  ...
  async function resumeExistingBooking(bookingId: string, restoredDate: string, restoredSlot: string, restoredDuration: number) {
    ...
    if (restoredDate && restoredSlot) {
      const slotList = await availabilityApi.getSlots(houseSitterId, restoredDate, restoredDuration || 1)
  ...
  async function sendPhoneVerificationOtpInline() {
    ...
    try {
      await houseSitsApi.updateMe({ phone: phone.trim() })
    ...
    Promise.all([
      ...
      .then(([houseSitterRes, houseSitRes, addressRes, authUserRes]) => {
        ...
        if (addresses.length > 0 && canApplyDefaults) {
          const defaultAddress = addresses.find((entry: HouseSitAddressRead) => entry.is_default) ?? addresses[0]
          setAddressMode('saved')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const serverDraft = (await bookingsApi.getFlowDraft(houseSitterId)).data
  ...
  }, [loading, continueDraft, continueBookingId, resetDraft, houseSitterId])
  ...
  useEffect(() => {
    if (!date || !houseSitterId) return
    const requestSeq = ++slotsRequestSeqRef.current
    setSlotsLoading(true)
    availabilityApi.getSlots(houseSitterId, date, duration)
      .then((r) => {
  ...
  }, [date, duration, houseSitterId, selectedSlot, step])

  useEffect(() => {
    if (!houseSitterId) return
    const requestSeq = ++datesRequestSeqRef.current
    setBookableDatesLoading(true)
    setBookableDates([])
    availabilityApi.getBookableDates(houseSitterId, duration, 28)
      .then((r) => {
  ...
  }, [houseSitterId, duration, date, step, selectedSlot])

  // Fetch price breakdown when duration changes
  useEffect(() => {
    if (!houseSitterId) return
    bookingsApi.previewPrice(houseSitterId, duration)
      .then(r => setBreakdown(r.data ?? null))
      .catch(() => { })
  }, [duration, houseSitterId])
  ...
  async function assertSelectedSlotStillAvailable() {
    ...
    const slotList = await availabilityApi.getSlots(houseSitterId, date, duration)
    const stillAvailable = (slotList.data ?? []).some((slot) => {
      const slotIso = normalizeToIsoDatetime(slot.start)
  ...
  async function cancelDraftAndRestart() {
    ...
    try {
      ...
      router.push(`/house-sits/book/${houseSitterId}?reset=1&step=1`)
    } catch (err: any) {
              ...
              <CardContent className="space-y-6">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <Button type="button" variant="outline" className="h-8 px-3 text-xs" onClick={() => router.push('/house-sits/profile')}>
                      Edit in profile
                    </Button>
                  ...
                  {missingAccountItems.length > 0 && (
                    <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        {!emailVerified && (
                          <Button type="button" variant="outline" onClick={() => router.push('/house-sits/profile')} className="h-8 px-3 text-xs">
            ...
            <Card className="border-slate-200">
              <CardContent className="px-5 pb-5 pt-6 text-center space-y-5 sm:px-8 sm:pb-8 sm:pt-6">
                ...
                <div className="space-y-2 mx-auto max-w-sm">
                  <Button className="w-full" onClick={() => router.push('/house-sits/dashboard')}>
                    Go to Dashboard
                  </Button>
                  <Button variant="outline" className="w-full" onClick={() => router.push('/house-sits/house-sitters')}>
