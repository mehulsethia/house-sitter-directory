...
function CheckoutForm({ booking, onSuccess }: { booking: BookingRead; onSuccess: () => void }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSubmitting(true)
    const returnUrl = `${window.location.origin}/house-sits/bookings/${booking.id}?payment=authorized`
    ...
    async function init() {
      try {
        ...
        if (!['draft', 'pending', 'accepted'].includes(nextBooking.status)) {
          ...
          router.push(`/house-sits/bookings/${bookingId}`)
          return
        }
      ...
      <div className="client-checkout-revamp space-y-7 md:space-y-9">
        ...
        <div className="mx-auto w-full max-w-5xl">
          <Button
            ...
            onClick={() => router.push(`/house-sits/bookings/${bookingId}`)}
          >
          ...
          <Card className="border-slate-200 bg-white/90">
            ...
            <CardContent>
              <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: 'stripe' } }}>
                <CheckoutForm booking={booking} onSuccess={() => router.push(`/house-sits/bookings/${bookingId}`)} />
              </Elements>
            </CardContent>
          </Card>
        </section>
