...
export async function GET(req: NextRequest, ctx: { params: Promise<{ houseSitterId: string }> }) {
  const { houseSitterId } = await ctx.params
  const page = Number(req.nextUrl.searchParams.get('page') ?? 1)
  const pageSize = Number(req.nextUrl.searchParams.get('page_size') ?? 20)
  const [reviews, total] = await reviewRepo.findByCleanerId(houseSitterId, page, pageSize)
