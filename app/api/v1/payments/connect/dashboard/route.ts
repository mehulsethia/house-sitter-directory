...
export const POST = requireHouseSitter(async (_req, _ctx, user) => {
  ...
  if (!houseSitter.stripeAccountId) return err('Stripe account not connected', 400)

  const loginLink = await stripe.accounts.createLoginLink(houseSitter.stripeAccountId)
