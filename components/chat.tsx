...
export function Chat({
  ...
  return (
    <div
      className={cn(
        'flex flex-col overflow-hidden rounded-2xl border border-[#e7dfd6] bg-white',
        fullHeight ? 'h-full min-h-0' : '',
      )}
      style={fullHeight ? undefined : { height: 420 }}
    >
      {/* Header */}
      <div className="shrink-0 border-b border-[#e7dfd6] bg-[#fcfaf8] px-4 py-3 text-sm font-semibold text-[#3f3429]">
        In-booking chat
      </div>

      {/* Message list */}
      <div className="flex-1 space-y-3 overflow-y-auto bg-[#fefdfb] p-4">
        {messages.length === 0 && (
              ...
              <div
                className={cn(
                  ...
                  isOwn
                    ...
                    : 'rounded-bl-sm border border-[#e7dfd6] bg-white text-foreground',
                )}
              >
                {msg.content}
                <p
      ...
      {/* Input */}
      <div className="shrink-0 border-t border-[#e7dfd6] bg-white p-3">
        {readOnly ? (
